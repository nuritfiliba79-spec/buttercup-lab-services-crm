...
### אזורי אחסון קבצים (Supabase Storage)

| אזור (bucket) | תוכן | מי מעלה | הלקוח רואה? |
|---|---|---|---|
| `client-uploads` · קבצי הלקוח | שרטוטים, תמונות, COA ששולח הלקוח | לקוח (לפרויקטים שלו), מנהלים | כן |
| `lab-reports` · דוחות מעבדה | דוחות ביניים וסופיים (PDF / Word / Excel) | מנהל מעבדה, מנהל מערכת | כן |
| `test-data` · נתוני בדיקות וניסויים | נתונים גולמיים, CSV, קבצי ניסוי | מבצע בדיקות (בדיקות המעבדה שלו), מנהלים | לא, פנימי |
| `lab-files` · קבצי פרויקט | שרטוטים, תמונות ו-COA של המעבדה | מנהלים | כן |

- כל האזורים פרטיים. הקבצים נשמרים בנתיב `projects/<project_id>/`, וכל קובץ אפשר לשייך לפרויקט, להזמנה או לבדיקה.
- מחיקה מותרת למנהל מערכת, או למי שהעלה את הקובץ.
- לכל אזור מוגדרת רשימת סוגי קבצים מותרים ומגבלת גודל.
